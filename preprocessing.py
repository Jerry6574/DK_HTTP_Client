...
def init_webdriver(chromedriver_path=r"lib\chromedriver"):
    """
...
def get_supplier_df(supplier_center='https://www.digikey.com/en/supplier-centers'):
    """
    Parse HTML of the supplier_center page and build a DataFrame of supplier data.
    :return: supplier_df, a DataFrame, has columns=['supplier_name', 'supplier_url', 'supplier_code', 'supplier_id']
    """
    browser = init_webdriver()
    browser.get(supplier_center)
    ...
    supplier_list = [[anchor.text.replace('/', '_'),
                      anchor.get_attribute('href')] for anchor in supplier_anchors]
    browser.quit()

    supplier_df = pd.DataFrame(supplier_list, columns=['supplier_name', 'supplier_url'])
    supplier_codes = []
...
def get_supplier_code(supplier_url):
    ...
    If no supplier code, return 'NaN'. 
    """
...
def get_pg_df():
    ...
    :return: pg_df, a DataFrame, has columns=['product_group', 'pg_url', 'pg_url_key', 'pg_id]
    """
...
def main():
    ...
    # supplier_df.to_excel(SUPPLIER_DF_PATH)
